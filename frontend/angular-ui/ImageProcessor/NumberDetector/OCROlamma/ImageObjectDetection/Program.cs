...
namespace ImageObjectDetection;
...
class Program
{
    ...
    {
        Console.WriteLine("=== Image Object Detection using Ollama ===\n");

        // Get the Image folder path (at the root level, same as ImageObjectDetection folder)
        ...
        foreach (string imagePath in imageFiles)
        {
            try
            {
                ...
                string result = await DetectObjectsAsync(base64Image, imagePath);

                // Print the result
                Console.WriteLine($"License Plate Number: {result.Trim()}");
    ...
    static async Task<string> DetectObjectsAsync(string base64Image, string imagePath)
    {
        ...
        var requestPayload = new
        {
            ...
            messages = new[]
            {
                ...
                {
                    role = "user",
                    content = "You are an OCR system. Read the license plate number character by character from left to right with extreme precision. Pay careful attention to distinguish between similar looking characters (R vs M, 8 vs 3, etc.). Output only the complete license plate text exactly as it appears with no spaces, explanations, or additional content.",
